...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    affine = x @ Wx + prev_h @ Wh + b.reshape(1, -1)
    next_h = np.tanh(affine)
    cache = (x, prev_h, Wx, Wh, affine, next_h)
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, prev_h, Wx, Wh, affine, next_h = cache
    daffine = (1 - next_h ** 2) * dnext_h
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0
    for i in range(T):
      prev_h, cache_now = rnn_step_forward(x[:, i, :], prev_h, Wx, Wh, b)
      h[:, i, :] = prev_h
...
def rnn_backward(dh, cache):
    ...
    N, T, H = dh.shape
    _, D = cache[0][0].shape
    dnext_h_0 = dh[:, -1, :].reshape(N, H)
    dnext_h_1 = np.zeros((N, H))
    dx = np.zeros((N, T, D))
    dWx = np.zeros((D, H))
    dWh = np.zeros((H, H))
    db = np.zeros((H,))
    for i in range(T-1, -1, -1):
      dnext_h = dnext_h_0 + dnext_h_1
      dx[:, i, :], dnext_h_1, dWx_now, dWh_now, db_now = rnn_step_backward(dnext_h, cache[i])
      dWx += dWx_now
      dWh += dWh_now
      db += db_now
      if i >= 1:
        dnext_h_0 = dh[:, i - 1, :].reshape(N, H)
    dh0 = dnext_h_1
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
