...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        denom_left = torch.tensor([sim(z_k, out[j])/tau for j in range(2*N) if j!=k])
        denom_right = torch.tensor([sim(z_k_N, out[j])/tau for j in range(2*N) if j!=k+N])

        l_left = -((sim(z_k, z_k_N)/tau).exp() / denom_left.exp().sum()).log()
        l_right = -((sim(z_k_N, z_k)/tau).exp() / denom_right.exp().sum()).log()
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = (
      (out_left * out_right).sum(dim=1, keepdim=True) / 
...
def compute_sim_matrix(out):
    ...
    sim_matrix = (
      torch.mm(out, out.T) / norm.view(-1,1) / norm.view(1,-1) 
      )
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    mask = torch.cat([mask[N:], mask[:N]])
    similarity_positive_paris = sim_matrix.masked_select(mask).view(2*N, -1) # [2*N, 1]
    ...
    numerator = torch.exp(similarity_positive_paris/tau)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
