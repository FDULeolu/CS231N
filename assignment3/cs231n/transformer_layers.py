...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = query.view(N, S, self.n_head, self.head_dim).transpose(1, 2)  # (N, n_head, S, head_dim)

        key = self.key(key)
        key = key.view(N, T, self.n_head, self.head_dim).transpose(1, 2)      # (N, n_head, T, head_dim)

        value = self.value(value)
        value = value.view(N, T, self.n_head, self.head_dim).transpose(1, 2)  # (N, n_head, T, head_dim)

        # Compute the scores
        scores = torch.matmul(query, key.transpose(-2, -1)) / (self.head_dim ** 0.5)  # (N, n_head, S, T)
        ...
        output = torch.matmul(attn_weights, value)  # (N, n_head, S, head_dim)

        # Concatenate heads
        output = self.proj(output.transpose(1, 2).contiguous().view(N, S, E))  # (N, S, E)
