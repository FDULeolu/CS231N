...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            ...
            logist, temporal_affine_cache = temporal_affine_forward(h, W_vocab, b_vocab)
            loss, dout = temporal_softmax_loss(logist, captions_out, mask)

            # backward
            dh, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dout, temporal_affine_cache)
            dout, dh_0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dh, rnn_cache)
            grads['W_embed'] = word_embedding_backward(dout, embed_cache)
            dx, grads['W_proj'], grads['b_proj'] = affine_backward(dh_0, affine_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        if self.cell_type == 'rnn':
            ...
            for i in range(max_length-1):
                ...
                logist, _ = affine_forward(prev_h, W_vocab, b_vocab)
                captions[:, i+1] = np.argmax(logist, axis=1)
